...
export async function charUpdatePrimaryWorld(name) {
    ...
    } else if (typeof context.saveCharacterDebounced === 'function') {
...
export async function setCharBindings(type, worldName, isEnabled) {
    ...
    if (type === 'primary') {
        ...
        // Remove from old primary if different (and exists)
    ...
    if (type === 'chat') {
        ...
        if (isEnabled) {
            ...
            // Unbind: remove current chat binding if exists
            if (oldChatWb) {
...
function updateBoundBooksCacheEntry(bookName, delta) {
    ...
    const applyDelta = (target) => {
        if (!target[bookName]) {
            target[bookName] = { primary: [], aux: [], global: false, chat: [] };
        }
        const entry = target[bookName];
    ...
    if (STATE.boundBooksCache && STATE.boundBooksCache.version === STATE.boundBooksCacheVersion) {
        applyDelta(STATE.boundBooksCache.data);
    }

    // Always update STATE.boundBooksSet directly to keep it in sync
    if (STATE.boundBooksSet) {
        applyDelta(STATE.boundBooksSet);
    } else {
        STATE.boundBooksSet = {};
        applyDelta(STATE.boundBooksSet);
    }
