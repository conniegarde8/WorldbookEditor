...
export const Actions = {
    ...
    async refreshAllContext() {
        try {
            ...
            // 后台加载绑定映射（现在改为 await 以确保完成后更新 STATE.boundBooksSet）
            try {
                STATE.boundBooksSet = await this.loadBoundBooksSetAsync();
    ...
    async saveBindings() {
        ...
        try {
            // Capture old STATE.bindings values
            const oldCharPrimary = STATE.bindings.char.primary;
            const oldCharAdditional = [...STATE.bindings.char.additional];
            const oldGlobal = [...STATE.bindings.global];
            const oldChat = STATE.bindings.chat;

            // Compute deltas
            const toRemove = oldGlobal.filter(b => !globalBooks.includes(b));
            const toAdd = globalBooks.filter(b => !oldGlobal.includes(b));

            // 1. Manual update of STATE.boundBooksSet for auxiliary changes (since charSetAuxWorlds won't trigger setCharBindings)
            const context = getContext();
            if (!STATE.boundBooksSet) STATE.boundBooksSet = {};
            const charName = context.characters[context.characterId]?.name;
            if (charName) {
                const addedAux = charAdditional.filter(name => !oldCharAdditional.includes(name));
                const removedAux = oldCharAdditional.filter(name => !charAdditional.includes(name));

                addedAux.forEach(book => {
                    if (!STATE.boundBooksSet[book]) STATE.boundBooksSet[book] = { primary: [], aux: [], global: false, chat: [] };
                    if (!STATE.boundBooksSet[book].aux.includes(charName)) STATE.boundBooksSet[book].aux.push(charName);
                });
                removedAux.forEach(book => {
                    if (STATE.boundBooksSet[book]) {
                        STATE.boundBooksSet[book].aux = STATE.boundBooksSet[book].aux.filter(name => name !== charName);
                    }
                });
            }

            // 2. Optimistically update STATE.bindings and UI
            ...
            // 3. Trigger actual persistence operations
            const promises = [];

            // Primary binding
            promises.push(setCharBindings('primary', charPrimary || '', !!charPrimary));

            // Global bindings: remove old, add new
            ...
            // Auxiliary binding: use charSetAuxWorlds directly
            ...
            if (charId || charId === 0) {
                ...
                promises.push(charSetAuxWorlds(charFileName, charAdditional));
            }

            // Wait for all operations to settle (they should be fast now)
            const results = await Promise.allSettled(promises);
            const errors = results.filter(r => r.status === 'rejected').map(r => r.reason);
            if (errors.length > 0) {
                logger.error('Binding save errors:', errors);
                toastr.error('部分绑定保存失败，请查看日志');
            } else {
    ...
    async toggleBindState(bookName, targetCharName, isUnbind) {
        ...
        if (isUnbind) {
            ...
            if (currentChar !== targetCharName) return; // cannot unbind others
            try {
                // Optimistic update
                STATE.bindings.char.primary = null;
                await setCharBindings('primary', bookName, false);
                UI.renderManageView();
            } catch (e) {
                // Rollback
                STATE.bindings.char.primary = bookName;
