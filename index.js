...
const API = {
    ...
    async fetchGlobalSettings() {
        // 只有当缓存为空或强制刷新时才请求，避免每次切换视图都 Fetch
        if (!STATE.globalSettingsCache) {
            ...
            if (response.ok) {
                STATE.globalSettingsCache = await response.json();
            } else {
                STATE.globalSettingsCache = {};
            }
        }
        return STATE.globalSettingsCache;
    },

    // --- 读取类 ---
    async getAllBookNames() {
        const context = getContext();
        // 优先尝试从 context 获取 (标准扩展 API)
        if (context.worldInfo && Array.isArray(context.worldInfo)) {
            return context.worldInfo.map(b => b.name).sort((a, b) => a.localeCompare(b));
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                // 根据 API 返回结构处理 (通常是 { world_info: [...] } 或数组)
                const list = data.world_info || data || [];
                if (Array.isArray(list)) {
                    return list.map(b => typeof b === 'string' ? b : b.name).sort((a, b) => a.localeCompare(b));
                }
            }
        } catch (e) {
            console.warn("[Worldbook Editor] Failed to fetch world names via API", e);
        }

        // 最后的回退方案：DOM 读取 (仅当处于世界书标签页时有效)
    ...
    async getCharBindings() {
        ...
        if (!charId && charId !== 0) return { primary: null, additional: [] };
    ...
    async getGlobalBindings() {
        // 优化：从缓存读取
        const settings = await this.fetchGlobalSettings();
        return settings.world_info?.globalSelect || [];
    },
    ...
    async createEntry(name, newEntriesArray) {
        ...
        const combined = [...currentEntries, ...newEntriesArray];
    ...
    // --- 绑定类 ---
    async setGlobalBindings(names) {
        const settings = await this.fetchGlobalSettings();
        if (!settings.world_info) settings.world_info = {};
        settings.world_info.globalSelect = names;
        ...
        // 更新本地缓存
        STATE.globalSettingsCache = settings;
        getContext().updateWorldInfoList();
    },

    async setCharBindings(data) {
        const context = getContext();
        const charId = context.characterId;
        ...
        if (data.primary !== undefined) {
             ...
             const char = currentContext.characters[charId];

             if (char && char.data) {
                 if (!char.data.extensions) char.data.extensions = {};
                 char.data.extensions.world = data.primary;
        ...
        // Additional: 更新 Settings
        if (data.additional !== undefined) {
            const settings = await this.fetchGlobalSettings();
            if (!settings.world_info) settings.world_info = {};
            if (!settings.world_info.charLore) settings.world_info.charLore = [];

            const charName = context.characters[charId].avatar.replace(/\.[^/.]+$/, "");
            const idx = settings.world_info.charLore.findIndex(e => e.name === charName);

            if (data.additional.length === 0) {
                if (idx !== -1) settings.world_info.charLore.splice(idx, 1);
            } else {
                if (idx !== -1) settings.world_info.charLore[idx].extraBooks = data.additional;
                else settings.world_info.charLore.push({ name: charName, extraBooks: data.additional });
            }
    ...
    async setChatBinding(name) {
        const context = getContext();
        if (!context.chatMetadata) return;
        context.chatMetadata.world_info = name || undefined;
...
const Actions = {
    async init() {
        ...
        es.on(et.SETTINGS_UPDATED, () => {
            STATE.globalSettingsCache = null; // 清空缓存，下次读取时自动刷新
    ...
    async refreshAllContext() {
        try {
            // 显示 Loading (如果是视图切换等操作触发的刷新)
            const loader = document.getElementById('wb-loading-layer');
            if (loader && !loader.style.display) loader.style.display = 'flex';
    ...
    async addNewEntry() {
        ...
        const newEntry = {
            ...
            key: [],             // 原生字段: key
            order: 100,          // 原生字段: order
            position: 1,         // 原生字段: 1 (after_char_def)
            depth: 4,            // 原生字段: depth
            probability: 100,
            selective: true,
            // 其他字段保持默认
    ...
    async saveBindings() {
        ...
        try {
            await Promise.all([
                API.setCharBindings({ primary: charPrimary || null, additional: charAdditional }),
                API.setGlobalBindings(globalBooks),
                API.setChatBinding(chatBook || null)
            ]);
            await this.refreshAllContext();
        } catch (e) {
        ...
        if (isUnbind) {
            ...
            try {
                if (currentChar === targetCharName) {
                    const currentBindings = await API.getCharBindings();
                    if (currentBindings.primary === bookName) {
                        await API.setCharBindings({ ...currentBindings, primary: null });
                    }
                }
                await this.refreshAllContext();
                UI.renderManageView();
            ...
            try {
                const currentBindings = await API.getCharBindings();
                await API.setCharBindings({ ...currentBindings, primary: bookName });
                await this.refreshAllContext();
                UI.renderManageView();
            } catch (e) {
    ...
    async actionExportTxt() {
        ...
        const processExport = (type) => {
            try {
                ...
                // 2. 排序 (按优先级: Order -> UID)
                targetEntries.sort((a, b) => (a.order ?? 0) - (b.order ?? 0) || a.uid - b.uid);
...
const UI = {
    ...
    open() {
        ...
        // [新增] 打开面板时立即刷新所有上下文数据，确保绑定信息是最新的
        // 使用 .catch 忽略可能的网络错误，不阻塞 UI 打开
        Actions.refreshAllContext().catch(e => console.error("Auto-refresh failed:", e));
                    ...
                    <div class="wb-book-bar">
                        ...
                        <div class="wb-menu-wrapper">
                            ...
                            <div class="wb-menu-dropdown" id="wb-analysis-menu">
                                ...
                                <div class="wb-menu-item" data-type="export_txt"><i class="fa-solid fa-file-lines"></i> 导出世界书TXT</div>
                            </div>
                        </div>
                        <div class="wb-menu-wrapper">
        ...
        if (STATE.currentBookName) {
            UI.renderList();
             ...
             // 尝试智能选中一本（如果在预加载阶段没有选中的话）
            if (STATE.allBookNames.length > 0) {
                 // 默认逻辑已在 Actions.init 处理，这里只需从 STATE 读取
                 if (STATE.entries.length === 0) {
                      UI.renderList(); // 渲染空列表或引导
                 }
            }
        }
    ...
    createCard(entry, index) {
        ...
        bind('.inp-pos', 'change', (e) => {
            ...
            // [修改] 仅控制深度输入框的显示，不再触碰宽度，宽度全由 CSS 接管
            if (depthInput) {
    ...
    applyCustomDropdown(selectId) {
        ...
        const update = () => {
            ...
            // [修改] 宽度控制权完全移交 CSS，此处不再进行任何 JS 样式覆盖
        };
        update();
        originalSelect.onchange = update;
    },
    ...
    openAnalysisModal() {
        ...
        const render = () => {
            ...
            // [修改] 进度条改为计算占总Token的比例，不再需要 maxTokens
        ...
        try {
            ...
            if (activatedEntries.length === 0) {
                ...
                activatedEntries.forEach((entry, idx) => {
                    ...
                    // [修改] 增加 data-color-mode 用于 CSS 样式控制
                    sidebarHtml += `
    ...
    initTooltips() {
        ...
        const show = (text, x, y, colorMode) => {
            ...
            tipEl.style.left = Math.min(x + 15, window.innerWidth - rect.width - 10) + 'px';
            tipEl.style.top = Math.min(y + 15, window.innerHeight - rect.height - 10) + 'px';
        };
        const hide = () => {
