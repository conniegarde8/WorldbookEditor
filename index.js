 ...
 * 替代新版 ST 的 charUpdatePrimaryWorld，直接操作数据以兼容旧版
 */
async function charUpdatePrimaryWorld(name) {
    ...
    if (uiSelect) {
        uiSelect.value = name;
        // 触发 change 事件以确保 UI 状态更新（部分版本需要）
 ...
 * 替代新版 ST 的 charSetAuxWorlds，直接操作 world_info 对象以兼容旧版
 */
...
const Actions = {
    ...
    async addNewEntry() {
        ...
        const newEntry = {
            ...
            key: [],             // 原生字段: key
            order: 0,            // 默认顺序改为0，配合数组前置，确保新建时出现在顶部
        ...
        overlay.innerHTML = `
            <div class="wb-export-card">
                <div class="wb-export-header">
                    <div class="wb-export-title">导出世界书为 TXT</div>
                    <div class="wb-export-close">×</div>
                </div>

                <!-- 分组 1 -->
                <div class="wb-export-section">
                    <div class="wb-export-label">导出所有条目</div>
                    <div class="wb-export-grid">
                        <button class="wb-export-btn" data-type="all-title">含标题</button>
                        <button class="wb-export-btn" data-type="all-no-title">不含标题</button>
                    </div>
                </div>

                <!-- 分组 2 -->
                <div class="wb-export-section">
                    <div class="wb-export-label">仅导出已启用条目</div>
                    <div class="wb-export-grid">
                        <button class="wb-export-btn" data-type="enabled-title">含标题</button>
                        <button class="wb-export-btn" data-type="enabled-no-title">不含标题</button>
                    </div>
                </div>
            </div>
        `;
...
const UI = {
    ...
    async open() {
        ...
        panel.innerHTML = `
            <div class="wb-header-bar">
                <div class="wb-tabs">
                    <div class="wb-tab-glider"></div> <!-- 新增滑动层 -->
        ...
        const createMultiSelect = (containerSelector, initialSelectedArray, dataClass) => {
            ...
            tagsEl.onclick = () => {
                ...
                if (!isVisible) {
                    ...
                    // 修复：仅在非触摸设备(PC)上自动聚焦，避免移动端自动弹键盘遮挡视线
    ...
    updateCardStatus(uid) {
        ...
        // [修复] 必须显式移除所有状态类，然后根据当前状态重新添加
        // 否则从 disabled 切换回 enabled 时，无法恢复 type-green/type-blue 样式
        card.classList.remove('disabled', 'type-green', 'type-blue');

        if (entry.disable) {
            card.classList.add('disabled');
        } else {
            // 如果启用了，判断是常驻(蓝)还是普通(绿)
            if (entry.constant) {
    ...
    toggleCustomDropdown(selectId, triggerElem) {
        ...
        // [优化] 仅在非触摸设备(PC)上自动聚焦，避免移动端自动弹键盘遮挡视线
        const isTouchDevice = window.matchMedia('(pointer: coarse)').matches;
        if (!isTouchDevice) {
            searchInput.focus();
        }
    ...
    openSortingModal() {
        // 立即移除按钮焦点，防止关闭弹窗后按钮保持黑色（hover/focus态）
        ...
        const overlay = document.createElement('div');
        overlay.className = 'wb-sort-modal-overlay';
        // 移除 visibility: hidden，利用 CSS 瞬间渲染
        ...
        // 移除 setTimeout 和 adjustSortModalLayout 调用
        ...
        // 不再需要 JS 定位，CSS Flexbox 已经处理了居中
    ...
    initSortableGroup(listEl, groupKey) {
        ...
        listEl.addEventListener('touchmove', (e) => {
            ...
            // [关键优化] 使用 requestAnimationFrame 节流，解决卡顿问题
            if (rAF) return;
            ...
            rAF = requestAnimationFrame(() => {
                ...
                // 核心逻辑：获取触点下的元素
                // elementFromPoint 因为 pointer-events:none 会穿透拖拽物
