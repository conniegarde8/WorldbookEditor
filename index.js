    ...
    async open() {
        ...
        panel.innerHTML = `
            <div class="wb-header-bar">
                <div class="wb-tabs">
                    <!-- [修复] 添加 no-transition 类，防止打开时的滑动延迟 -->
                    <div class="wb-tab-glider no-transition"></div>
        ...
        // 初始化视图状态
        // [修复] 此时 glider 拥有 no-transition 类，这次 update 会瞬间完成，无视觉延迟
        UI.updateGlider('editor');
        Actions.switchView('editor');

        // [修复] 下一帧移除禁止动画类，恢复后续交互的丝滑效果
        requestAnimationFrame(() => {
            // 双重 rAF 确保 DOM 绘制已完成
            requestAnimationFrame(() => {
                const glider = document.querySelector('.wb-tab-glider');
                if (glider) glider.classList.remove('no-transition');
            });
        });
    },
