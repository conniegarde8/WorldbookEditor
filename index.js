...
const Actions = {
    ...
    async loadBook(name) {
        ...
        // [优化] 立即清空内存数据，防止在网络慢时 UI 显示上一本书的残留内容
        STATE.entries = [];
...
const UI = {
    ...
    async open() {
        ...
        // 在打开面板时，检查缓存的 currentBookName 是否仍然有效
        // 防止用户在关闭面板期间，在原生 UI 中删除了该书，导致插件显示僵尸数据
        if (STATE.currentBookName && !STATE.allBookNames.includes(STATE.currentBookName)) {
            console.warn(`[Worldbook] Detected stale book name: ${STATE.currentBookName}, resetting.`);
            STATE.currentBookName = null;
            STATE.entries = [];
        }
    ...
    openSortingModal() {
        ...
        groupKeys.forEach((key, i) => {
            ...
            // --- 移动端触摸拖拽逻辑 (手动模拟 - 长按图标触发) ---
            let touchDraggingItem = null;
            let longPressTimer = null;

            listEl.addEventListener('touchstart', (e) => {
                // 1. 严格限制触发区域：必须是"三道杠"图标
                // 注意：e.target 可能是图标内部的 path，所以用 closest 找 fa-bars
                const handle = e.target.closest('.fa-bars');
                if (!handle) return; // 如果触摸的不是图标，直接返回，允许原生滚动
            ...
            listEl.addEventListener('touchmove', (e) => {
                ...
                // A. 如果长按还没触发（touchDraggingItem 为空） -> 视为用户想取消长按或进行页面滚动
